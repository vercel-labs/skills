...
const COGNITIVE_TYPES_YAML = join(ROOT, 'config/cognitive-types.yaml');
const AGENT_TYPE_OUT = join(ROOT, 'src/core/__generated__/agent-type.ts');
const COGNITIVE_TYPES_OUT = join(ROOT, 'src/core/__generated__/cognitive-types.ts');
...
// ============================================
// Cognitive Types Loading & Generation
// ============================================

function loadCognitiveTypes(): Record<string, CognitiveTypeEntry> {
  const content = readFileSync(COGNITIVE_TYPES_YAML, 'utf-8');
  const data = parseYaml(content) as Record<string, CognitiveTypeEntry>;

  const errors: string[] = [];
  for (const [key, entry] of Object.entries(data)) {
    if (!entry.subdir) errors.push(`${key}: missing required field "subdir"`);
    if (!entry.fileName) errors.push(`${key}: missing required field "fileName"`);
  }

  if (errors.length > 0) {
    console.error('Cognitive types validation errors:');
    for (const e of errors) console.error(`  - ${e}`);
    process.exit(1);
  }
...
function generateCognitiveTypesFile(types: Record<string, CognitiveTypeEntry>): string {
  const keys = Object.keys(types);
  const unionMembers = keys.map((k) => `'${k}'`).join(' | ');

  const subdirsEntries = keys.map((k) => `  ${k}: '${types[k]!.subdir}',`).join('\n');
  const fileNameEntries = keys.map((k) => `  ${k}: '${types[k]!.fileName}',`).join('\n');

  return `// AUTO-GENERATED by scripts/compile-agents.ts — DO NOT EDIT
// Source: config/cognitive-types.yaml

export type CognitiveType = ${unionMembers};
...
function generateAgentsFile(
  ...
  cognitiveTypes: Record<string, CognitiveTypeEntry>
): string {
  ...
  const ctKeys = Object.keys(cognitiveTypes);
  ...
  for (const a of agents) {
    ...
    // Generate dirs Record dynamically from cognitive types
    const dirEntries = ctKeys.map((ct) => {
      const subdir = cognitiveTypes[ct]!.subdir;
      const localPath = localDir(a.localRoot, subdir);
      const globalPath = globalDirExpr(a.globalRoot, subdir);
      return `      ${ct}: { local: '${localPath}', global: ${globalPath} },`;
    });
...
function main() {
  ...
  console.log(
    `Loaded ${Object.keys(cognitiveTypes).length} cognitive types from config/cognitive-types.yaml`
  );
  ...
  // Generate files
  const cognitiveTypesContent = generateCognitiveTypesFile(cognitiveTypes);
  const agentTypeContent = generateAgentTypeFile(resolved);
  const agentsContent = generateAgentsFile(resolved, cognitiveTypes);
  ...
  writeFileSync(COGNITIVE_TYPES_OUT, cognitiveTypesContent);
  writeFileSync(AGENT_TYPE_OUT, agentTypeContent);
  writeFileSync(AGENTS_OUT, agentsContent);

  console.log(`Generated ${COGNITIVE_TYPES_OUT}`);
